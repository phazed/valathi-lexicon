...
(() => {
  ...
  const PANEL_CLASS = "monster-vault-panel";
  let _mvCtx = { panelEl: null, labelEl: null };
  function _mvSetCtx(ctx) {
    const panelEl = (ctx && (ctx.panelEl || ctx.panel)) || document.getElementById("generatorPanel");
    const labelEl = (ctx && (ctx.labelEl || ctx.label)) || document.getElementById("activeGeneratorLabel");
    _mvCtx = { panelEl, labelEl };
  }
  function _mvGetPanel() {
    return _mvCtx.panelEl || document.getElementById("generatorPanel");
  }
  function _mvGetLabel() {
    return _mvCtx.labelEl || document.getElementById("activeGeneratorLabel");
  }
  function _mvRenderFatal(err) {
    const panel = _mvGetPanel();
    const label = _mvGetLabel();
    if (label) label.textContent = TOOL_NAME;
    if (!panel) return;
    const msg = (err && (err.stack || err.message)) ? String(err.stack || err.message) : String(err);
    panel.innerHTML = `
      <div style="border:1px solid #4a2b2b;background:#160b0b;border-radius:10px;padding:10px;color:#ffd0d0;">
        <div style="font-weight:700;margin-bottom:6px;">Monster Vault crashed while rendering.</div>
        <div style="font-size:0.85rem;opacity:0.9;white-space:pre-wrap;">${esc(msg)}</div>
        <div style="margin-top:8px;font-size:0.8rem;opacity:0.85;">Open DevTools → Console for details.</div>
      </div>
    `;
    try { console.error("[MonsterVault] render error", err); } catch (_) {}
  }
  ...
  function renderMonsterVaultTool(ctx) {
    _mvSetCtx(ctx);
    const panel = _mvGetPanel();
    if (panel) {
      // Ensure the panel is in a clean state (other tools may leave styling behind)
    ...
    try {
      return _mvRenderMonsterVaultToolCore();
    } catch (err) {
      _mvRenderFatal(err);
    }
  }

function _mvRenderMonsterVaultToolCore() {
    const panel = _mvGetPanel();
    const label = _mvGetLabel();
    ...
    if (search) {
      search.addEventListener("input", () => {
        ...
        const freshSearch = _mvGetPanel()?.querySelector("#mvSearch");
        if (freshSearch) {
          try {
